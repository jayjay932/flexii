...
$listing_id = isset($_GET['vehicule_id']) ? (int)$_GET['vehicule_id'] : 0;

// Vérifier si `listing_id` est valide
if (!$listing_id) {
    echo json_encode(['error' => 'Missing vehicule_id']);
    exit;
}
...
$stmt = $pdo->prepare("SELECT host_id FROM vehicules WHERE id = ?");
$stmt->execute([$listing_id]);
$listing = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$listing) {
    echo json_encode(['error' => 'vehicule not found']);
    exit;
}
