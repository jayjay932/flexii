      ...
      <main className="dashboard-container">
        ...
        <div className="charts-grid">
          ...
          <div className="chart-box">
            ...
            <p style={{ marginBottom: '0.5rem' }}>
              {logementsOccupes}% des nuits occupées ce mois-ci
            </p>
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie
        ...
        <section className="section">
          ...
          {Array.isArray(demandes) && demandes.length > 0 ? (
            <ul className="list">
              {demandes.map((d) => (
                <li key={d.id} className={`list-item ${d.statut}`}>
                  <span className="list-icon">{renderStatutIcon(d.statut)}</span>
                  <div className="list-content">
                    <strong>{d.titre}</strong>
                    <span className="badge-status">
                      {d.statut === 'valide' ? 'Validé' : d.statut === 'refuse' ? 'Refusé' : 'En attente'}
                    </span>
                    ...
                    <div className="action-buttons">
                      <button className="btn-valid" onClick={() => updateStatut(d.id, 'valide')}>
                        Valider
                      </button>
                      <button className="btn-refuse" onClick={() => openRefuseModal(d.id)}>
                        Refuser
                      </button>
                    </div>
                  </div>
                ...
                {logements.map((l) => {
                  ...
                  return (
                    <tr key={l.id}>
                      ...
                      <td>{l.voyageurs}</td>
                      <td>{l.prix} €</td>
                      <td><span className={`status-badge ${l.statut}`}>{l.statut}</span></td>
                      <td>{ca} €</td>
                    </tr>
                  );
                })}
