...
function ListingDetails() {
  ...
  // 📌 Calcul du texte de l'offre jour gratuit
  const margin = 6000;
  let offerText = null;
  if (listing && listing.price_per_night) {
    const proprietorPrice = listing.price_per_night - margin;
    const daysRequired = Math.ceil(proprietorPrice / margin);
    const freeDay = daysRequired + 1;
    const ordinal = freeDay === 1 ? 'er' : 'e';
    offerText = `Réservez ${daysRequired} jour${daysRequired > 1 ? 's' : ''} et le ${freeDay}${ordinal} jour est offert.`;
  }

  // ✅ Fonction d'affichage du prix selon le type
  const getListingPriceText = (l) => {
    if (l.rental_type === 'courte' && l.price_per_night) return `${l.price_per_night} FCFA / nuit`;
    if (l.rental_type === 'mensuel' && l.price_per_month) return `${l.price_per_month} FCFA / mois`;
    if (l.rental_type === 'achat' && l.price_for_sale) return `${l.price_for_sale} FCFA / vente`;
    return 'Prix non spécifié';
  };

  // 🔄 Données liées à l’annonce
  useEffect(() => {
  ...
  return (
    <div className="listing-detail">
      ...
      {showGallery && <FullGallery images={listing.images} onClose={() => setShowGallery(false)} />}
  ...
  <div className="price">{getListingPriceText(listing)}</div>

  {listing.rental_type === 'achat' ? (
    <Link to={`/formulaire-achat?id=${listing.id}`} className="btn">Réserver</Link>

  ) : listing.rental_type === 'mensuel' ? (
    <Link to={`/formulaire-mensuel/${listing.id}`} className="btn">Réserver</Link>
  ) : (
    <button className="btn" onClick={() => setCalendarOpen(true)}>Réserver</button>
  )}
</div>


        {offerText && (
          <div className="offer-text" style={{ color: 'hotpink', marginTop: '8px', fontWeight: 'bold' }}>
            {offerText}
          </div>
        )}
        ...
        <div className="rules-card" style={{ marginTop: '50px' }}>
          <h2 className="section-title">Conforts supplémentaires</h2>
          <ul className="rules-list">
        ...
        {validComments.length > 0 ? (
          <section style={{ marginTop: 50 }}>
            ...
            <p style={{ color: '#666', marginBottom: 30, maxWidth: 650 }}>
              Ce logement fait partie des Coups de coeur voyageurs, d’après les avis.
            </p>
              ...
              {validComments.map((c, i) => (
                <div key={i} className="comment-card">
                  <div className="comment-text-wrapper">
                    ...
                    <a href="#" className="read-more-link" onClick={e => { e.preventDefault(); e.target.previousSibling.style.webkitLineClamp = 'unset'; e.target.style.display = 'none'; }}>Lire la suite</a>
                  </div>
            ...
            <div style={{ textAlign: 'center', marginTop: 40 }}>
              <Link to={`/afficher_commentaires/${listing.id}`} className="btn" style={{ padding: isMobile ? '8px 16px' : '12px 24px', fontSize: isMobile ? '14px' : '16px' }}>
        ...
        {loadingHost ? (<div>Chargement des informations de l'hôte...</div>) : host ? (
          <div className="host-card">
            <div className="host-left">
              <div className="host-avatar-container">
                <img src={host.avatar_url ? `http://localhost/flexii/api/${host.avatar_url}` : '/default-avatar.jpg'} alt={`Photo de ${host.name}`} className="host-avatar" />
